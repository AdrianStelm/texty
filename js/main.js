...
formEditNote.addEventListener('submit', () => {
    ...
    if (currentEditingTitle) {
        localStorage.removeItem(currentEditingTitle); // Видаляємо стару нотатку
    }

    localStorage.setItem(newTitle, newText); // Додаємо оновлену нотатку
    modalWindowForEditNote.classList.remove('active'); // Закриваємо модалку
    renderNotes(); // Перемальовуємо нотатки
})
